...
const changeIndex = (state, index, backBehavior, initialRouteName) => {
  ...
  if (backBehavior === 'history' || backBehavior === 'fullHistory') {
    ...
    if (backBehavior === 'history') {
      ...
      history = history.filter(it => it.type === 'route' ? it.key !== currentRoute.key : true);
      ...
      if (lastHistoryRouteItem?.type === 'route' && currentRoute.key === lastHistoryRouteItem.key) {
    ...
    history = [...state.history.filter(it => it.type !== 'route'), ...getRouteHistory(state.routes, index, backBehavior, initialRouteName)];
  }
  return {
    index,
    history
...
export function SwitchRouter({
  ...
  const router = {
    ...
    getRehydratedState(state, {
      ...
      return {
        ...stateData,
        ...changeIndex(stateData, index, backBehavior, initialRouteName)
      };
    },
    getStateForRouteNamesChange(state, {
      ...
      if (index === -1) {
        ...
        const previousRouteItem = history.findLast(item => item.type === 'route');
        index = Math.max(0, routes.findIndex(route => route.key === previousRouteItem?.key));
      }
      return {
        ...state,
        routeNames,
        ...
        preloadedRouteKeys: state.preloadedRouteKeys.filter(key => routeKeys.has(key))
      };
    },
    getStateForRouteFocus(state, key) {
      ...
      return {
        ...state,
        ...changeIndex(state, index, backBehavior, initialRouteName)
      };
    },
        ...
        case 'NAVIGATE_DEPRECATED':
          {
            ...
            const nextRoute = params !== route.params || path !== route.path || key !== route.key ? {
              ...route,
              key,
              path,
              params
            } : route;
            const routes = state.routes.map((item, routeIndex) => routeIndex === index ? nextRoute : item);
            const updatedState = {
              ...state,
              routes,
              history: key === route.key ? state.history : state.history.filter(it => it.type !== 'route' || it.key !== route.key)
            };
            return {
              ...updatedState,
              ...changeIndex(updatedState, index, backBehavior, initialRouteName),
              preloadedRouteKeys: updatedState.preloadedRouteKeys.filter(key => key !== route.key && key !== nextRoute.key)
            };
          }
        case 'SET_PARAMS':
        case 'REPLACE_PARAMS':
          {
            ...
            if (nextState !== null) {
              ...
              if (index != null) {
                ...
                const historyItemIndex = state.history.findLastIndex(item => item.type === 'route' && item.key === focusedRoute.key);
        ...
        case 'PRELOAD':
          {
            ...
            let history = state.history;
            if (key !== route.key) {
              history = history.filter(record => record.type !== 'route' || record.key !== route.key);
              if (routeIndex === state.index) {
                history = history.concat({
                  type: TYPE_ROUTE,
                  key: newRoute.key,
                  params: backBehavior === 'fullHistory' ? newRoute.params : undefined
                });
              }
            }
            return {
