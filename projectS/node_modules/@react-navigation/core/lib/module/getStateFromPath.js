...
const getStaticSegmentPattern = segment => Array.from(segment, char => {
  const encoded = encodeURIComponent(char);
  const percentEncoded = encoded === char ? `%${char.charCodeAt(0).toString(16).padStart(2, '0').toUpperCase()}` : encoded;
...
export function getStateFromPath(path, options) {
  ...
  let firstDecodedSegment;
  try {
    firstDecodedSegment = firstRawSegment != null ? decodeURIComponent(firstRawSegment) : undefined;
  } catch {
  ...
  const {
    routes,
    config
  } = matchAgainstConfigs(remaining, firstDecodedSegment, firstRawSegment, configs, configsByScreen);
...
function prepareConfigResources(options) {
  ...
  if (prefix) {
    ...
    const prefixPattern = normalizedPrefix.split('/').map(getStaticSegmentPattern).join('/');
    prefixRegex = new RegExp(`^${prefixPattern}`);
  }
  const configsByScreen = {};
  const configsByPattern = new Map();
...
function getSortedNormalizedConfigs(initialRoutes, screens = {}) {
  ...
  return [].concat(...Object.keys(screens).map(key => createNormalizedConfigs(key, screens, initialRoutes, [], [], []))).map((config, order) => ({
    ...config,
    order
  })).sort((a, b) => {
    ...
    if (isArrayEqual(a.segments, b.segments)) {
      if (a.routeNames.length > b.routeNames.length && arrayStartsWith(a.routeNames, b.routeNames)) {
        return -1;
      }
      if (b.routeNames.length > a.routeNames.length && arrayStartsWith(b.routeNames, a.routeNames)) {
        return 1;
      }
...
function checkForDuplicatedConfigs(existing, config, pattern) {
  if (!existing) {
    return;
  }
  ...
  const intersects = a.length > b.length ? arrayStartsWith(a, b) : arrayStartsWith(b, a);
  if (!intersects) {
...
const matchAgainstConfigs = (remaining, firstDecodedSegment, firstRawSegment, configs, configsByScreen) => {
  let routes;
...
const createConfigItem = (screen, routeNames, paths, parse, hasNestedScreens = false) => {
  ...
  const regex = parts.length ? new RegExp(`^(${parts.map((it, i) => {
    if (it.param) {
      const reg = it.regex ? `(?:${it.regex})|(?=[^/]*%[0-9A-F]{2})[^/]+` : '[^/]+';
...
const createNestedStateObject = (path, routes, initialRoutes, routeConfig) => {
  ...
  const params = parseQueryParams(path, routeConfig?.parse, routeConfig?.pathParamNames, routeConfig?.explicitParamNames, routeConfig?.hasNestedScreens, route.params);
  if (params) {
    route.params = {
      ...route.params,
      ...params
